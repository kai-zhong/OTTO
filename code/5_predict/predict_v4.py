...
def predict(type_name, test_features, FEATURES, co_matrix_list, oof_dict):
    ...
    for i, lgb_model in enumerate(model_paths):
        ...
        # for j in tqdm(range(0, num_samples, chunk_size), desc="正在进行分块预测"):
        #     # 获取当前批次的索引范围
        #     start_idx = i
        #     end_idx = min(i + chunk_size, num_samples)

        #     # 提取当前批次的数据（只选择特征列）
        #     X_batch = test_features.iloc[start_idx:end_idx][FEATURES]
        ...
        # result += (np.concatenate(predictions, axis=0)) / 5
        # print(f"模型{i} 分块预测完成。")
    ...
    # print(f'正在保存{type_name}的预测结果...')

    # if type_name != 'click_all':
    #     pred.to_parquet(f'{output_path}{type_name}_test_makotu_v4.parquet')
    #     print(f'结果已保存到{output_path}{type_name}_test_makotu_v4.parquet.')
    # else:
    #     pred.to_parquet(f'{output_path}click_test_makotu_v4_all_target.parquet')
    #     print(f'结果已保存到{output_path}click_test_makotu_v4_all_target.parquet.')
...
if __name__ == '__main__':
    ...
    for type_name in ['click', 'click_all', 'cart', 'order']:
        ...
        if type_name == 'click_all':
            output_file_path = f'{output_path}click_test_v4_all_target.parquet'
        else:
            output_file_path = f'{output_path}{type_name}_test_v4.parquet'
        ...
        if type_name != 'click_all':
            test_pred.to_parquet(f'{output_path}{type_name}_test_v4.parquet')
            print(f'结果已保存到{output_path}{type_name}_test_v4.parquet.')
        else:
            test_pred.to_parquet(f'{output_path}click_test_v4_all_target.parquet')
            print(f'结果已保存到{output_path}click_test_v4_all_target.parquet.')